        ...
        for(int i=1; i< campArray.size()+1; i++){
            ...
            if(decision == 0){
                ...
                switch(choice_modi){
                    ...
                    */
                    case 6:
                        if(!campArray.get(countKeep.indexOf(decision)).getCampStudentList().isEmpty() || !campArray.get(countKeep.indexOf(decision)).getCampCommitteeList().isEmpty()){
                        ...
                        switch(chooseModif){
                            case 1:
                                System.out.println("Changed Visibility to Visible only to Staff");
                                campArray.get(countKeep.indexOf(decision)).setVisiblility(Camp.visibilityStatus.NOVISIBILE);
                                
                                break;
                            case 2:
                                System.out.println("Changed Visibility to be seen in Faculty only!");
                                campArray.get(countKeep.indexOf(decision)).setVisiblility(Camp.visibilityStatus.FACULTY);
                                break;
                            case 3:
                                System.out.println("Changed Visibility to the entire school");
                                campArray.get(countKeep.indexOf(decision)).setVisiblility(Camp.visibilityStatus.ENTIRESCHOOL);
                                break;
                            default:
