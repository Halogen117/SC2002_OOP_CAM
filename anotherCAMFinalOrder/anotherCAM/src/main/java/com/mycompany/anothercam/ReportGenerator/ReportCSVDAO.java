...
public class ReportCSVDAO {
    ...
    private ArrayList<Camp> campArray;
    private boolean printCommittee;
    private boolean printAttendee;
    private boolean printOnlyCreate;
    ...
    * @param tempCampArray The ArrayList containing the camp Array to be printed out (Only contains the camp created by staff)
    ...
    */
    public ReportCSVDAO(ArrayList<Camp> tempCampArray, ArrayList<Camp> campArray, String filename, boolean printCommittee, boolean printAttendee, boolean printOnlyCreate){
        this.filename = filename;
        this.tempCampArray = tempCampArray;
        this.printCommittee = printCommittee;
        this.printAttendee = printAttendee;
        this.printOnlyCreate = printOnlyCreate;
    ...
    */
    public boolean writeStaffReportInfo(){
        try{
            ...
            for(int i=1; i< useCampArray.size()+1; i++){
                String slotString = String.valueOf(useCampArray.get(i-1).getTotalSlot());
                String committeeString = String.valueOf(useCampArray.get(i-1).getCampCommitteeSlots());
                String firstCommittee;
                String firstAttendee;
                // Print the first row of new Camp
                if(this.printCommittee == true){
                    if(useCampArray.get(i-1).getCampCommitteeList().size() <=0){
                        firstCommittee = "NIL";
                    }else{
                        firstCommittee = useCampArray.get(i-1).getCampCommitteeList().get(0).getUserName();
                    }
                    String[] firstCol = {String.valueOf(i) ,useCampArray.get(i-1).getCampName(), useCampArray.get(i-1).getDateString(),useCampArray.get(i-1).getRegClosingDateString(),slotString ,committeeString,useCampArray.get(i-1).getStaffInCharge(), firstCommittee, ""};
                    this.csvFile.writeNext(firstCol);
                
                
                // Print other camp committee members
                for(int j=1; j< useCampArray.get(i-1).getCampCommitteeList().size(); j++){
                    String[] secondData = {"", "", "", "", "", "", "", useCampArray.get(i-1).getCampCommitteeList().get(j).getUserName(), ""};
                    this.csvFile.writeNext(secondData);
                }
                }else{
                    String[] firstCol = {String.valueOf(i) ,useCampArray.get(i-1).getCampName(), useCampArray.get(i-1).getDateString(),useCampArray.get(i-1).getRegClosingDateString(),slotString ,committeeString,useCampArray.get(i-1).getStaffInCharge(), "", ""};
                    this.csvFile.writeNext(firstCol);
                }
                
                if(this.printAttendee == true){
                    if(useCampArray.get(i-1).getCampStudentList().size() <=0){
                        firstAttendee = "NIL";
                    }else{
                        firstAttendee = useCampArray.get(i-1).getCampStudentList().get(0).getUserName();
                    }
                        String[] teriaryData = {"", "", "", "", "", "", "", "", firstAttendee};
                        this.csvFile.writeNext(teriaryData);
                    for(int k=1; k< useCampArray.get(i-1).getCampStudentList().size(); k++){
                        String[] secondaryTeriaryData = {"", "", "", "", "", "", "", "", useCampArray.get(i-1).getCampStudentList().get(k).getUserName()};
        ...
        try{
            ...
            for(int i=1; i< useCampArray.size()+1; i++){
                ...
                if(useCampArray.get(i-1).getCampCommitteeList().size() <=0){
                    ...
                    firstCommittee = useCampArray.get(i-1).getCampCommitteeList().get(0).getUserName();
                    firstCommitteePoints = Integer.toString(useCampArray.get(i-1).getCampCommitteeList().get(0).getPoints());
                }
                String[] firstCol = {String.valueOf(i) ,useCampArray.get(i-1).getCampName(), useCampArray.get(i-1).getDateString(),firstCommittee, firstCommitteePoints};
                this.csvFile.writeNext(firstCol);
                // Print other camp committee members
                for(int j=1; j< useCampArray.get(i-1).getCampCommitteeList().size(); j++){
                    String[] secondData = {"", "", "", useCampArray.get(i-1).getCampCommitteeList().get(j).getUserName(), Integer.toString(useCampArray.get(i-1).getCampCommitteeList().get(j).getPoints())};
                    this.csvFile.writeNext(secondData);
                }
            String[] blankCol = {"", "", "", "", ""};
            this.csvFile.writeNext(blankCol);
