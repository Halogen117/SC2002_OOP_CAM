...
public class StaffCampCreate{
    ...
    */
    public boolean runStaffCreateCamp(ArrayList<Camp> campArray, Staff cookie, Scanner scanObj){
        int campID;
        ...
        if(cookie instanceof Staff){
            if(checkStaffMadeCamp(campArray, cookie)){
                System.out.println("Unable to make another camp as you had already made one earlier!");
                return false;
            }
            ...
            date = this.dateFormat.generateDate(scanObj);
            if(date == null){
            ...
            reg_closing_date = this.dateFormat.generateDate(scanObj);
            ...
            if(reg_closing_date == null){
                ...
                return false;
            }else if(this.dateFormat.compareDateAndTime(reg_closing_date, this.today) == 1){
                System.out.println("The registration date of the camp cannot be earlier than the predefined timing of today!");
                return false;
            }
            ...
            total_slot = total_slot + CampCommitteeSlots;
            ...
            Camp new_camp = new Camp(campID, campName, date, reg_closing_date, location, total_slot, CampCommitteeSlots, description, staff_in_charge, visibility, faculty);
    ...
    */
    ...
    /**
    * Checks if a Staff has created a camp earlier in the Camp Array.
    * @param campArray the CampArray which the new camp object is stored.
    * @param staffCookie the user cookie used to create the camp.
    * @return the boolean value whether a camp created by the same user was done.
    */
    public boolean checkStaffMadeCamp(ArrayList<Camp> campArray, Staff staffCookie){
        for(int i=0; i< campArray.size(); i++){
            if(campArray.get(i).getStaffInCharge().equals(staffCookie.getUserID())){
                return true;
